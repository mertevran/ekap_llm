...
def _epsilla_retrieve_with_profile_vectors(
    *,
    epsilla_client,
    ...
    """
    Üretim FAISS getirim yönünü Epsilla üzerinde taklit eder.

    Akış:
        profil FAISS hazır vektörleri
        -> Epsilla TenderChunks
        -> tender_id / IKN filtresi
        -> chunk_id bazında en iyi sonuç
        -> en iyi N chunk
        -> mevcut ScoreAggregator
    """
    import json
    import math
    ...
    normalized_profile = str(profile_code).strip().upper()

    reader = FaissVectorReader()

    profile_entries = reader.find_profile_entries(
        profile_vector_store,
        normalized_profile,
    )
    ...
    if not profile_entries:
        raise RuntimeError(
            f"Profil FAISS indeksinde profil bulunamadı: {normalized_profile}"
        )
    ...
    if not composite_query:
        raise RuntimeError(
            f"Profil FAISS metni boş: {normalized_profile}"
        )

    query_terms = _query_terms(composite_query)

    raw_by_chunk: dict[str, dict[str, Any]] = {}
    vector_diagnostics: list[dict[str, Any]] = []
    ...
    for vector_no, (faiss_id, profile_payload) in enumerate(
        profile_entries,
        start=1,
    ):
        vector = reader.reconstruct_vector_by_external_id(
            profile_vector_store,
            faiss_id,
        )
        ...
        vector_diagnostics.append({
            ...
            "section": str(profile_payload.get("section") or ""),
        ...
        # Önce tender_id filtresi.
        try:
            raw_results = epsilla_client.query(
        ...
        # Filtre desteklenmezse geniş havuzdan Python filtresi.
        if not raw_results:
            all_results = epsilla_client.query(
                table_name="TenderChunks",
                query_field="embedding",
                query_vector=vector,
                limit=search_top_k * 4,
                response_fields=response_fields,
            )
            ...
            raw_results = [
                ...
                if (
                    str(row.get("tender_id") or "").strip() == tender_id
                    or str(row.get("ikn") or "").strip() == ikn
                )
            ]
        ...
        for row in raw_results:
            ...
            # Mevcut Epsilla deney sözleşmesi:
            # küçük @distance daha iyi.
            score = (
                0.0
            ...
            normalized_row["_epsilla_distance"] = (
                None if not math.isfinite(distance) else distance
            )
            normalized_row["_score"] = score
            normalized_row["_profile_vector_no"] = vector_no
            normalized_row["_profile_faiss_id"] = int(faiss_id)

            current = raw_by_chunk.get(chunk_id)

            if (
                current is None
                or score > float(current.get("_score", 0.0))
            ):
                raw_by_chunk[chunk_id] = normalized_row

    if not raw_by_chunk:
        return None, {
            "retrieval_mode": "profile_guided",
            "profile_code": normalized_profile,
            "profile_vector_count": len(profile_entries),
            "profile_vectors": vector_diagnostics,
            "raw_unique_chunk_count": 0,
            "selected_chunks": [],
        }
    ...
    top_chunks = unique_chunks[:max_chunks_per_tender]
    ...
    for chunk in top_chunks:
        for field_name in ("okas_codes", "profile_codes"):
            ...
            if isinstance(raw_value, str):
                try:
                    chunk[field_name] = json.loads(raw_value)
                except Exception:
                    chunk[field_name] = []

    raw_scores = [
        float(chunk.get("_score", 0.0))
        for chunk in top_chunks
    ]
    ...
    for chunk in top_chunks:
        values = chunk.get("okas_codes") or []
    ...
    breakdown = scorer.compute(
        ...
        profile_okas_prefixes=profile_meta.get(
            "okas_prefixes", []
        ),
        strong_terms=profile_meta.get(
            "strong_terms", []
        ),
        negative_terms=profile_meta.get(
            "negative_terms", []
        ),
    ...
    top_payload = top_chunks[0]

    profile_codes_raw = top_payload.get("profile_codes", [])
    if not isinstance(profile_codes_raw, list):
        profile_codes_raw = []
    ...
    result = TenderSearchResult(
        ...
        tender_name=tender_name,
        chunk_title=(
            evidence_chunks[0].chunk_title
            if evidence_chunks
            else ""
        ),
        primary_profile_code=str(
            top_payload.get("primary_profile_code")
            or normalized_profile
        ),
        profile_codes=[
            str(value)
            for value in profile_codes_raw
        ],
        classification_status=str(
            top_payload.get("classification_status")
            or ""
        ),
        idare_adi=_first_text(
            top_payload.get("authority"),
    ...
    diagnostics = {
        "retrieval_mode": "profile_guided",
        "profile_source": "existing_python_classifier",
        "vector_source": "faiss_profile_index_read_only",
        "profile_code": normalized_profile,
        "profile_vector_count": len(profile_entries),
        "profile_vectors": vector_diagnostics,
        "raw_unique_chunk_count": len(unique_chunks),
        "selected_chunks": [
            {
                "rank": rank,
                "chunk_id": str(chunk.get("chunk_id") or ""),
                ...
                "epsilla_distance": chunk.get(
                    "_epsilla_distance"
                ),
                "score": float(
                    chunk.get("_score", 0.0)
                ),
                "matched_profile_vector_no": chunk.get(
                    "_profile_vector_no"
                ),
                "matched_profile_faiss_id": chunk.get(
                    "_profile_faiss_id"
                ),
            }
...
class _OllamaDiagnosticsCapture(logging.Handler):
    """
    OllamaDecisionModel'in [DIAGNOSTICS] logunu deney raporuna taşır.
    Üretim model koduna müdahale etmez.
    """
...
def _serialize_validation_issues(validation: Any) -> list[dict[str, Any]]:
    """Validator sorunlarını JSON raporuna güvenli biçimde aktarır."""
    ...
    for issue in issues:
        if hasattr(issue, "model_dump"):
            try:
                result.append(issue.model_dump(mode="json"))
                continue
            except Exception:
                pass
...
def run_epsilla_test(
    ...
    # -----------------------------------------------------------------------
    # [SELECTION] Profil belirlenebilen ilk N aktif, İSBAK dışı ihaleyi seç
    #
    # Epsilla profil seçmez.
    # Profil seçimi mevcut IsbakTenderProfileClassifier ile yapılır.
    # Amaç Qwen karar hattına gerçekten girebilecek ilk N gerçek ihaleyi
    # Epsilla retrieval üzerinden değerlendirmektir.
    # -----------------------------------------------------------------------
    ...
    try:
        ...
        for batch_start in range(
            ...
            for ikn in batch_ikns:
                ...
                primary = str(
                    classification.primary_profile_code or ""
                ).strip()
        ...
        logger.info(
            f"[SELECTION] Karar hattına uygun test kümesi hazır | "
    ...
    for idx, tender in enumerate(selected_tenders, 1):
        ...
        try:
            # ----------------------------------------------------------------
            # [PROFILE_MATCH] Mevcut sistemin profil sınıflandırıcısını kullan
            #
            # Epsilla burada profil SEÇMEZ.
            # Profil seçimi mevcut IsbakTenderProfileClassifier sonucudur.
            # Epsilla yalnız ihale chunk retrieval (getirim) katmanıdır.
            # ----------------------------------------------------------------
            ...
            secondary_profiles = [
                str(code).strip()
                for code in (classification.profile_codes[1:] or [])
                if str(code).strip()
                and str(code).strip() != primary_profile
            ]

            review_profiles = list(
                classification.review_profile_codes or []
            )

            evaluation_profiles = list(
                classification.evaluation_profile_codes or []
            )

            # Epsilla retrieval sorgusu için ihale içeriğini kullan.
            okas_list_tender = getattr(tender, "okas_codes", []) or []

            okas_text = " ".join(
                str(getattr(code, "ad", "") or "").strip()
                for code in okas_list_tender
                if str(getattr(code, "ad", "") or "").strip()
            )

            kapsam = str(
                getattr(tender, "kapsam", "") or ""
            ).strip()
            ...
            timer.end(
                ...
                {
                    "source": "existing_python_classifier",
            ...
            logger.info(
                f"[PROFILE_MATCH] IKN={ikn} | "
                f"source=existing_python_classifier | "
            ...
            entry["profile_match_source"] = "existing_python_classifier"

            if primary_profile == "BELIRLENEMEDI":
                entry.update(
                    final_decision="inceleme_gerekli",
                    final_confidence=0.0,
                    reason="Mevcut profil sınıflandırıcısı profil belirleyemedi",
                    evidence_chunk_ids=[],
                )
                ...
                logger.info(
                    ...
                    "inceleme_gerekli | profil belirlenemedi"
                )
            ...
            if "_epsilla_profile_store" not in locals():
                _epsilla_profile_store = FaissVectorStore(
                    path=str(_PROJECT_ROOT / "storage" / "faiss_profiles"),
                    collection_name="isbak_company_profiles",
                )

                if not _epsilla_profile_store.collection_exists():
                    raise FileNotFoundError(
                        "Profil FAISS indeksi bulunamadı: "
                        f"{_epsilla_profile_store.index_file}"
                    )

                if _epsilla_profile_store.index is None:
                    raise RuntimeError(
                        "Profil FAISS indeksi belleğe yüklenemedi."
                    )

                logger.info(
                    f"[PROFILE_VECTOR_STORE] "
                    f"path={_epsilla_profile_store.index_file} | "
                    f"vectors={_epsilla_profile_store.index.ntotal}"
                )

            tender_result, profile_guided_diag = (
                _epsilla_retrieve_with_profile_vectors(
                    epsilla_client=epsilla_client,
                    profile_vector_store=_epsilla_profile_store,
                    profile_code=primary_profile,
                    tender_id=tender_id,
                    ikn=ikn,
                    profile_meta=profile_meta,
                    scorer=epsilla_retriever.scorer,
                    max_chunks_per_tender=(
                        epsilla_retriever.max_chunks_per_tender
                    ),
                    search_top_k=epsilla_retriever.top_k,
                )
            )

            logger.info(
                f"[EPSILLA_PROFILE_GUIDED] IKN={ikn} | "
                f"profile={primary_profile} | "
                f"profile_vectors="
                f"{profile_guided_diag.get('profile_vector_count', 0)} | "
                f"unique_chunks="
                f"{profile_guided_diag.get('raw_unique_chunk_count', 0)}"
            )

            for selected_chunk in profile_guided_diag.get(
                "selected_chunks", []
            ):
                logger.info(
                    f"[EPSILLA_PROFILE_CHUNK] "
                    f"IKN={ikn} | "
                    f"rank={selected_chunk.get('rank')} | "
                    f"chunk_id={selected_chunk.get('chunk_id')} | "
                    f"section={selected_chunk.get('section')} | "
                    f"distance="
                    f"{selected_chunk.get('epsilla_distance')} | "
                    f"score={selected_chunk.get('score')}"
                )
            ...
            if "_ollama_diag_capture" not in locals():
                _ollama_diag_capture = _OllamaDiagnosticsCapture()
                logging.getLogger(
                    "app.decision.ollama_decision_model"
                ).addHandler(_ollama_diag_capture)

            _ollama_diag_capture.clear()
            ...
            # Qwen diagnostics — Ollama [DIAGNOSTICS] logundan yakalanır.
            captured_diag = (
                _ollama_diag_capture.last()
                if "_ollama_diag_capture" in locals()
                else {}
            )

            qwen_diag: dict[str, Any] = {
                "model": captured_diag.get("model"),
                "prompt_version": captured_diag.get("prompt"),

                "load_duration_ns": captured_diag.get(
                    "load_duration"
                ),
                "prompt_eval_duration_ns": captured_diag.get(
                    "prompt_eval_duration"
                ),
                "eval_duration_ns": captured_diag.get(
                    "eval_duration"
                ),
                "total_duration_ns": captured_diag.get(
                    "total_duration"
                ),

                "prompt_eval_count": captured_diag.get(
                    "prompt_eval_count"
                ),
                "eval_count": captured_diag.get(
                    "eval_count"
                ),
                "done_reason": captured_diag.get(
                    "done_reason"
                ),
                "response_length": captured_diag.get(
                    "response_length"
                ),
                "num_ctx": captured_diag.get(
                    "num_ctx"
                ),
                "num_predict": captured_diag.get(
                    "num_predict"
                ),
                "prompt_tokens_per_second": captured_diag.get(
                    "prompt_tokens_per_second"
                ),
                "generation_tokens_per_second": captured_diag.get(
                    "generation_tokens_per_second"
                ),
            }

            # Kullanışlı saniye karşılıkları.
            for ns_key, seconds_key in (
                ("load_duration_ns", "load_duration_s"),
                (
                    "prompt_eval_duration_ns",
                    "prompt_eval_duration_s",
                ),
                ("eval_duration_ns", "eval_duration_s"),
                ("total_duration_ns", "total_duration_s"),
            ):
                ns_value = qwen_diag.get(ns_key)
                qwen_diag[seconds_key] = (
                    round(float(ns_value) / 1_000_000_000, 6)
                    if isinstance(ns_value, (int, float))
                    else None
                )

            validation_issues = _serialize_validation_issues(
