...
def test_faiss_distant_logic(tmp_path):
    ...
    with patch("sys.argv", test_args):
        with patch("scripts.run_tender_decision_chain.FaissVectorStore") as mock_fvs_cls, \
            ...
            mock_store.payloads = {
                100: {"ikn": "2024/111", "tender_id": "1", "idare_adi": "İSBAK İSTANBUL BİLİŞİM", "text": "isbak"},
            ...
            def mock_index_reconstruct(pid):
                ...
                if pid == 200: return np.array([0.0, 0.0, 1.0], dtype=np.float32)
                if pid == 300: return np.array([1.0, 0.0, 0.0], dtype=np.float32)
                if pid == 400: return np.array([0.0, 0.5, 0.5], dtype=np.float32)
            ...
            def mock_reconstruct_profile_vector(faiss_id):
                if "PROFILE_A" in faiss_id: return np.array([1.0, 0.0, 0.0], dtype=np.float32)
            ...
            assert summary["distant_tenders_examined"] == 4
